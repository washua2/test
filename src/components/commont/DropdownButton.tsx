...
const DropdownRadioButton: React.FC<selfProps> = (props) => {
  ...
  const handleChange = useCallback(
    (event: any, item: any, index: number) => {
      setCount(countIndex + 1);
  ...
  return (
    <div className="relative" ref={divRef}>
      <button
        ...
        className="w-[12.5rem] h-[4.5rem] pl-5 pr-4 flex cursor-pointer items-center justify-between 
      ...
      >
        <div>
          <span className="text-right text-[1.625rem] font-bold font-Inter text-white">
            {text}
          </span>
        </div>
        <svg
